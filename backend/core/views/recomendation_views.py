...
def add_new_intervention(request):
    ...
    try:
        ...
        logger.error(f"[add_new_intervention] Unexpected error: {str(e)}", exc_info=True)
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
...
def list_intervention_diagnoses(request, intervention, specialisation, therapist_id):
    ...
    try:
        ...
        logger.error(f"[list_intervention_diagnoses] Unexpected error: {str(e)}", exc_info=True)
        return JsonResponse({'error': str(e)}, status=500)
...
def assign_intervention_to_types(request):
    ...
    try:
        ...
        existing_entry = next((rec for rec in therapist.default_recommendations if rec.recommendation == intervention), None)
...
def remove_intervention_from_types(request):
    ...
    try:
        ...
        intervention = Intervention.objects.get(id=ObjectId(data.get("intervention_id")))
        ...
        logger.error(f"[remove_intervention_from_types] Unexpected error: {str(e)}", exc_info=True)
        return JsonResponse({'error': 'Invalid JSON body'}, status=400)
    except Exception as e:
        logger.error(f"[remove_intervention_from_types] Unexpected error: {str(e)}", exc_info=True)
        return JsonResponse({'error': str(e)}, status=500)
    ...
    try:
        ...
        for pt in intervention.patient_types:
            if pt['diagnosis'] == diagnosis and pt['type'] == spec_type:
                return JsonResponse({'success': False, 'message': 'Diagnosis already exists'}, status=400)
        ...
        logger.error(f"[create_patient_group] Unexpected error: {str(e)}", exc_info=True)
