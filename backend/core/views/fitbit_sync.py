...
def fetch_fitbit_date_range_for_user(user, start_date: datetime.date, end_date: datetime.date) -> int:
    ...
    for dt in sorted(d for d in all_dates if start_date <= d <= end_date):
        ...
        elif dt not in wear_time_map:
            logger.warning("[fitbit_range] intraday HR unavailable for user=%s date=%s — wear_time not set", user, dt)
...
def fetch_fitbit_today_for_user(user, bypass_cooldown: bool = False) -> int:
    ...
    def wear_time_for(dt: datetime.date) -> int | None:
        ...
        if not dataset:
            logger.warning("[fitbit] intraday HR unavailable for user=%s date=%s — wear_time not set", user, dt)
            return None
        worn_minutes = {entry["time"][:5] for entry in dataset if entry.get("value", 0) > 0}
        return len(worn_minutes) if worn_minutes else None
