...
from core.models import (
    User, SMSVerification, Therapist, Patient, FeedbackQuestion, FeedbackEntry, Translation,
    AnswerOption, Intervention, RehabilitationPlan, PatientICFRating, PatientType,
)
from mongoengine import get_connection, connect, disconnect
from core.models import PatientType
...
def test_therapist_and_patient_relationship():
    user = User(username='therapist1', email='t1@example.com', phone='12345', createdAt=datetime.now())
    user.save()
    ...
    patient_user = User(username='patient1', email='p1@example.com', phone='67890', createdAt=datetime.now())
    patient_user.save()
...
def test_feedback_question_with_translations_and_answers():
    ...
    options = [AnswerOption(
        key='yes',
        translations=[Translation(language='en', text='Yes')]
    )]
...
def test_intervention_and_patient_icf_rating():
    ...
    user.save()
    therapist = Therapist(userId=user, name='T', first_name='T', specializations=['Cardiology'], clinics=['Downtown Clinic']).save()
    patient = Patient(
        userId=user,
        name='Pat',
        first_name='Test',
