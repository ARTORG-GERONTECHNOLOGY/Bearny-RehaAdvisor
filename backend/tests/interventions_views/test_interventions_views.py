...
from core.models import User, Therapist, Intervention, PatientType
from core.models import DefaultInterventions, PatientInterventionLogs
client = Client()
...
def create_therapist_and_intervention():
    user = User(username='therapist', email='t@example.com', phone='123', createdAt='2023-01-01').save()
    therapist = Therapist(userId=user, name='Therapist', first_name='Test', specializations=['Cardiology'],
                           clinics=['Downtown Clinic'], default_recommendations=[])
    therapist.save()
