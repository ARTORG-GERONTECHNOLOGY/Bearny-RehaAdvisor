...
from core.models import User, Patient, Therapist, Intervention, RehabilitationPlan, InterventionAssignment, PatientInterventionLogs
from bson import ObjectId
from core.models import DefaultInterventions, FeedbackQuestion, Translation
client = Client()
...
def setup_patient_with_plan():
    ...
    therapist_user = User(username='t1', email='t1@example.com', phone='123', createdAt=datetime.now())
    therapist_user.save()
    therapist = Therapist(userId=therapist_user, name='Doe', first_name='John', specializations=['Cardiology'], clinics=['Downtown Clinic'])
    therapist.save()

    # Create Patient
    patient_user = User(username='p1', email='p1@example.com', phone='456', createdAt=datetime.now())
    patient_user.save()
    patient = Patient(
        userId=patient_user,
        name='Patient',
...
def test_submit_feedback_success_intervention(mock_getattr, mongo_mock):
    ...
    payload = {
        ...
        "responses": [
            {"question": "How did it go?", "answer": ["Great"]}
        ]
    }
