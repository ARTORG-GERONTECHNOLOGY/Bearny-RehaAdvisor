...
const MetricBarOrBox = forwardRef<SVGSVGElement, Props>((props, ref) => {
  ...
  useEffect(() => {
    ...
    if (useFitbitRange) {
      ...
      const band = personalRangeFromWindow(data, (d) => d.date, accessor, {
      ...
      drawRangeLineSeries(svgRef, rows, res as 'weekly' | 'monthly', t(titleKey), band, {
        legend: {
      ...
      drawBoxTimeseries(svgRef, rows, res as 'weekly' | 'monthly', t(`${titleKey} (distribution)`));
    }
  }, [
    data,
    accessor,
