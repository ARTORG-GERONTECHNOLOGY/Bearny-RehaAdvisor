...
function ymKey(d: Date) { return `${d.getFullYear()}-${(d.getMonth()+1).toString().padStart(2,'0')}`; }
...
const AdherenceLine = forwardRef<SVGSVGElement, Props>(({ data, res, start, end }, ref) => {
  ...
  useEffect(() => {
    ...
    if (res === 'daily') {
      rows = inRange.map(r => ({ d: new Date(r.date), y: r.pct }));
    } else if (res === 'weekly') {
      ...
      rows = Array.from(map.values())
        ...
        .map(v => ({ d: v.d, y: v.sched > 0 ? Math.round(100 * v.comp / v.sched) : null }));
    } else { // monthly
      const map = new Map<string, { sched: number; comp: number; d: Date }>();
      inRange.forEach(r => {
        const d = new Date(r.date);
        const key = ymKey(d);
        if (!map.has(key)) map.set(key, { sched: 0, comp: 0, d: new Date(d.getFullYear(), d.getMonth(), 1) });
        const v = map.get(key)!;
        v.sched += r.scheduled;
        v.comp  += r.completed;
      });
    ...
    // Dimensions / scales
    const W = 800, H = 300, M = { top: 30, right: 20, bottom: 40, left: 48 };
    svg.attr('viewBox', `0 0 ${W} ${H}`);

    const x = d3.scaleTime()
      .domain(d3.extent(rows, r => r.d) as [Date, Date])
