...
const RestingHRChart = forwardRef<HTMLDivElement, Props>(({ data, start, end, className }, ref) => {
  ...
  if (!hasReadings) {
    return (
      <ChartEmptyState
        ref={ref}
        message={t('No resting heart rate data')}
        hint={deviceEmpty ? t('hint_resting_hr_empty') : undefined}
        className={className}
      />
    );
  }
