...
// CSV export
// ─────────────────────────────────────────────────────────
export const buildHealthCsvBlob = (
  ...
  if (selections.adherence) {
    ...
    const out = rows.map((r) => [toEuroDate(r.label), r.sched, r.comp, r.pct != null ? r.pct : '']);
  ...
  if (selections.questionnaire) {
    ...
    for (const e of qIn) {
      const questionText =
        e.questionTranslations?.find((x) => x.language === lang)?.text ||
      ...
      const answerTexts = (e.answers || [])
        .map((a) => {
          const text =
            a.translations?.find((x) => x.language === lang)?.text ||
...
// ─────────────────────────────────────────────────────────
type Section =
  | { type: 'chart'; ref: RefObject<HTMLDivElement>; key: string; title: string }
  | { type: 'questionnaire'; key: 'questionnaire'; title: string };

export const buildHealthPdf = async (
  store: HealthPageStore,
  svgRefs: SvgRefs,
  from: Date,
  to: Date,
